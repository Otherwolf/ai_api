...
class Web(BaseConnection):
    async def connection(self):
        ...
        if not self.dispatcher.web_urls and not is_health_checker and not metrics:
            self.logger.error("Web-workers don`t define url!")
            return False
        routes = []
        ...
        if is_health_checker:  # Добавляем url для проверки работоспособности
            ...
            self.logger.debug(f"Initialization of check working ({url_health=} {url_live=})")
        ...
        if metrics:
            ...
            self.logger.debug(f"Init metrics: {', '.join(metrics)}")
        self._myapp.add_routes(routes)
        return True

    def run_app(self):
        self.logger.info("Web inited successful!")
